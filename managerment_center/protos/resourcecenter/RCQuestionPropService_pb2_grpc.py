...
class RCQuestionPropServiceStub(object):
  ...
  def __init__(self, channel):
    ...
    self.listExerciseType = channel.unary_unary(
        '/resourcecenter.RCQuestionPropService/listExerciseType',
        request_serializer=google_dot_protobuf_dot_empty__pb2.Empty.SerializeToString,
        response_deserializer=RCCommon__pb2.ListResp.FromString,
        )
...
class RCQuestionPropServiceServicer(object):
  ...
  def listExerciseType(self, request, context):
    """查询练习题类(结果message QuestionProp)
    """
    context.set_code(grpc.StatusCode.UNIMPLEMENTED)
    context.set_details('Method not implemented!')
    raise NotImplementedError('Method not implemented!')
...
def add_RCQuestionPropServiceServicer_to_server(servicer, server):
  rpc_method_handlers = {
      ...
      'listExerciseType': grpc.unary_unary_rpc_method_handler(
          servicer.listExerciseType,
          request_deserializer=google_dot_protobuf_dot_empty__pb2.Empty.FromString,
          response_serializer=RCCommon__pb2.ListResp.SerializeToString,
      ),
